...
app.post("/generate", async (req, res) => {
  try {
    ...
    if (!prompt || prompt.trim().length < 3) return res.status(400).json({ success: false, error: "Prompt too short" });
    ...
    let html = full.replace(/^```html\s*/i, "").replace(/^```\s*/i, "").replace(/\s*```\s*$/i, "").trim();
...
app.post("/edit", async (req, res) => {
  try {
    ...
    if (!html || !instruction) return res.status(400).json({ success: false, error: "Missing params" });
    ...
    let edited = full.replace(/^```html\s*/i, "").replace(/^```\s*/i, "").replace(/\s*```\s*$/i, "").trim();
    const di = edited.search(/<!doctype html>/i);
    if (di > 0) edited = edited.substring(di);
    res.write(`data: ${JSON.stringify({ type: "done", html: edited })}\n\n`);
    res.end();
    console.log("✅ Edit done:", edited.length);
  } catch (err) {
    console.error("❌", err?.message);
    res.write(`data: ${JSON.stringify({ type: "error", message: err?.message || "Edit failed" })}\n\n`);
    res.end();
  }
});
